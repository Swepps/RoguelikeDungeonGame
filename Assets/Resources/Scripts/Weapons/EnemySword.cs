    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        for (int i = 0; i < collisionTags.Length; i++)
        {
            if (collision.tag == collisionTags[i])
            {
                ...
                {
                    collision.GetComponent<Player>().DealDamage(Random.Range(minDamage, maxDamage), transform.position, knockback);
                    if (bleeds)
                        collision.GetComponent<Player>().InflictDebuff(Player.Debuff.BLEED, 3);
                }
                break;
            }
        }
    ...
    override protected void Update()
    {
        if (isShooting)
        {
            if (shootReady)
            {
                ...
                swingDir = target.position - transform.parent.position;
